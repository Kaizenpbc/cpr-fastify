    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {filteredCourses.length === 0 ? (
        <Box sx={{ bgcolor: '#fff', border: '1px solid #E5E7EB', borderRadius: '10px', p: 6, textAlign: 'center' }}>
          <Typography sx={{ fontSize: 14, fontWeight: 600, color: '#9CA3AF' }}>No archived courses found</Typography>
        </Box>
      ) : (
        <DataTable columns={columns} shownCount={filteredCourses.length} totalCount={courses.length}>
          {filteredCourses.map((course) => (
            <DataTableRow key={course.id} columns={columns}>
              ...
              <Typography sx={{ fontSize: 13, color: '#4B5563' }}>{course.instructor || '—'}</Typography>
              <Typography sx={{ fontSize: 13, color: '#111827' }}>{course.studentsAttended || 0} / {course.registeredStudents}</Typography>
              <StatusChip kind={getStatusKind(course.status)} label={course.status.charAt(0).toUpperCase() + course.status.slice(1).replace('_', ' ')} />
              <Typography sx={{ fontSize: 13, color: '#4B5563' }}>{formatDisplayDate(course.confirmedDate || '')}</Typography>
              <Typography sx={{ fontSize: 13, color: '#4B5563' }}>{formatDisplayDate(course.archivedAt || '')}</Typography>
      ...
      <Dialog open={studentDialogOpen} onClose={handleCloseStudentDialog} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          {loadingStudents ? (
            ...
            <Typography sx={{ fontSize: 13, color: '#9CA3AF' }}>No students found for this course.</Typography>
          ) : (
